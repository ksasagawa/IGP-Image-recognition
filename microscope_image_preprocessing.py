...
def test_threshold(image, skip, lower_mask=[0,0,0], upper_mask=[179,255,250], 
                   k_size = 7, t_min = 0, t_max = 255, t_type = 3):
    lower_mask = np.asarray(lower_mask)
    upper_mask = np.asarray(upper_mask)
...
def square_cut(image, glob, iter_num, left_bound=0, right_bound=3072, top=0, bottom=2048):
    ...
    for i in g_cnts:
            ...
            cv2.drawContours(im_c3, approx,-1,(0,255,0),-1)
            ...
            if len(approx) == 4 and cv2.contourArea(i) > square_lower_bound and cv2.contourArea(i) < square_upper_bound:
                #draw it 
                cv2.drawContours(image,g_cntrRect,-1,(0,255,0),-1)
                ...
                x,y,w,h = cv2.boundingRect(approx)
                cv2.putText(imclone, str(cv2.contourArea(approx)), (x, y-5), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.9, (36,255,12), 2)
...
def square_cut_save(image, glob, iter_num, save, today, left_bound=0, right_bound=3072, top=0, bottom=2048):
    ...
    for i in g_cnts:
            ...
            if len(approx) == 4 and cv2.contourArea(i) > square_lower_bound and cv2.contourArea(i) < square_upper_bound:
                x,y,w,h = cv2.boundingRect(approx)
                if save:
                    cv2.imwrite(today + "/" + str(iter_num) + "-" + str(img_num) + ".jpg", 
                                image[y:y+h, x:x+w])
                rect_x.append(x)
                rect_y.append(y)
                avg_h += h
...
def main():
    ...
    for path in paths:
        ...
        #TODO
        if save or skip:
            cuts = square_cut_save(image, glob, iter_numb, save, today, L_bound, R_bound, Top_bound, Bottom_bound)
            total_cuts += cuts
        else:
            print(L_bound,R_bound,Top_bound,Bottom_bound)
    ...
    print("FINAL COUNT: \n" + str(total_cuts) + " slices out of " + str(iter_numb * 16))
